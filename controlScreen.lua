...
local function listenForTurtles()
    ...
    while not stopSignal do
        local _, message, protocol = rednet.receive("turtlePosData", 1)
...
local function listenForInput()
    ...
    while not stopSignal do
        ...
        if (string.find(input, "return")) then
            rednet.broadcast({ title = "turtleAubReturn" }, "turtleAubCommand")
            term.write("Forcing all turtles to return to origin.")
        end
        if (string.find(input, "mine deep")) then
            rednet.broadcast({ title = "turtleAubMineDeep" }, "turtleAubCommand")
            term.write("Commanded all to Mine Deep.")
        end
        if (input == "go") then
            rednet.broadcast({ title = "turtleAubGo" }, "turtleAubCommand")
